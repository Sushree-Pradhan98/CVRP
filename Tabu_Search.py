...
class TabuSearchCVRP:
    def __init__(self, locations, demands, vehicle_capacity, tabu_tenure=10, max_evaluations=10000):
    ...
    def tabu_search(self):
        ...
        while self.evaluations < self.max_evaluations:
            ...
            distances = [self.total_distance(sol) for sol in neighborhood]
            sorted_neighbors = [x for _, x in sorted(zip(distances, neighborhood))]

            # Log stats for convergence
    ...
    def plot_convergence(self):
        ...
        plt.ylabel('Fitness')
        plt.title('Fitness Convergence: Tabu Search')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.show()
